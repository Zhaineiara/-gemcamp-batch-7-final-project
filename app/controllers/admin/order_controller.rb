class Admin::OrderController < ApplicationController
  ...
  def index
    @orders = Order.includes(:offer, :user).order(created_at: :desc).page(params[:page]).per(10)
    ...
    if params[:start_date].present? || params[:end_date].present?
      ...
      @orders = @orders.where('created_at >= ?', start_date.beginning_of_day).page(params[:page]).per(10) if start_date
      @orders = @orders.where('created_at <= ?', end_date.end_of_day).page(params[:page]).per(10) if end_date
    end
