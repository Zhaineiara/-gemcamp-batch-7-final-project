class PhLocationService
  ...
  def fetch_cities
    ...
    data.each do |city|
      ...
      if address_city.province.nil?
        puts "Warning: Province not found for city #{city['name']} (Code: #{city['code']})"
      end

      if address_city.save
        puts "Saved city: #{address_city.name} (Code: #{address_city.code})"
      else
        puts "Failed to save city: #{address_city.errors.full_messages.join(", ")}"
      end
    end
  end
